#_*_coding:gb2312
import requests, xlrd, MySQLdb, time, sys,os,gettime
from xlutils import copy #从xlutils模块中导入copy这个函数
reload(sys)
sys.setdefaultencoding('GBK')
#print sys.getdefaultencoding() #打印系统默认的编码格式
localpath = os.getcwd()
...
def interfaceTest(case_list,file_path):
    ...
    for case in case_list:
        ...
        if method.upper() == 'GET':    # 如果是get请求就调用requests模块的get方法，.text是获取返回报文，保存返回报文，把返回报文存到返回报文的list中
            ...
            results = results.encode('GBK','ignore')  #转为GBK编码  ，在对unicode字符编码时，添加ignore参数，忽略无法无法编码的字符，这样就可以正常编码为GBK了                 
            ...
            results = results.encode('GBK','ignore')
...
#下面两段是我调试时用的，后来没有用上，保留以后查看方式   
    ...
    for request_url,response,flag in zip(request_urls,responses,res_flags):
        ...
        if len(response)<32767:                  # 如果返回报文小于32767，就把返回报文写入到EXCL结果中
            sheet.write(i,9,u'%s'%response)    # 在第9列写入返回报文的list  如果返回的报文太长会报错的，所有有时候就把这条给注释了，但无法看到返回的报文
        else:
            response = response[0:32767]
            sheet.write(i,9,u'返回的报文太长了,截断了部分%s'%response)
